<template>
    ...
    <div v-else class="grid space-x-2 gap-x-3 justify-center h-auto mx-28 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1">
        <div class="flex justify-center items-center my-3  transition-all hover:scale-105 rounded "
            v-for="rec in result.recipe" :key="rec.id">
            <div class="">
                ...
                <div class="">
                    <div>
                        ...
                        <!-- <div v-for="(instruction,index) in JSON.parse(rec.instructions).split(',')" :key="instruction"><span
                              class="text-2xl items-center">step {{index}}</span> {{ instruction }} </div> -->
                        <div class="flex space-x-3">
                            ...
                            <button @click="managefavorite(rec.id)">{{favorite.isfavorite}}
                                <svg v-if="!favorite.isfavorite" style="color: red" xmlns="http://www.w3.org/2000/svg" width="32"
                            ...
                            <!-- <button v-if="isfavorite" @click="removefavorite">
                          </button> -->
                        </div>
                    </div>
                </div>
...
<script setup >
import apolloClient from '../../stores/apolloclient.js';
...
const { error, loading, result } = useQuery(recipequery);
...
const managefavorite = (id) => {
    ...
    onMounted((id)=>favorite.checkfavorite(id))
    // console.log();
    if (!favorite.isfavorite) {
        ...
        isnotfavorite.value = true
    }
}
// const { mutate: makefav } = useMutation(addtofav)
// const addtofavorite = (id) => {
//     console.log(id);
//     user.addtofavorite({ recipeid: id })
//     isnotfavorite.value = false
