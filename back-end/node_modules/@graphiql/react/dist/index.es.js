...
import { isNonNullType, isListType, GraphQLError, parse as parse$1, TypeInfo, visit, print, getNamedType, isLeafType, Kind, visitWithTypeInfo, typeFromAST, GraphQLFloat, getIntrospectionQuery, buildClientSchema, validateSchema } from "graphql";
...
async function importCodeMirror(addons, options) {
  ...
  const allAddons = (options == null ? void 0 : options.useCommonAddons) === false ? addons : commonCodeMirrorAddons.concat(addons);
...
const formatSingleError = /* @__PURE__ */ __name((error2) => Object.assign(Object.assign({}, error2), { message: error2.message, stack: error2.stack }), "formatSingleError");
...
var TextEditChangeImpl = function() {
  ...
  TextEditChangeImpl2.prototype.replace = function(range2, newText) {
    this.edits.push(TextEdit.replace(range2, newText));
  };
  TextEditChangeImpl2.prototype.delete = function(range2) {
    this.edits.push(TextEdit.del(range2));
  };
...
function ExecutionContextProvider(props) {
  ...
  const run = useCallback(async (_selectedOperationName) => {
    var _a;
    ...
    const selectedOperationName = _selectedOperationName || (() => {
      var _a2;
      ...
      for (const operation of queryEditor.operations) {
        if (operation.loc && operation.loc.start <= cursorIndex && operation.loc.end >= cursorIndex) {
          return (_a2 = operation.name) == null ? void 0 : _a2.value;
        }
      }
      return void 0;
    })();
    ...
    if (selectedOperationName && selectedOperationName !== operationName) {
      ...
      onEditOperationName == null ? void 0 : onEditOperationName(selectedOperationName);
    }
    if (externalFragments) {
      const externalFragmentsMap = /* @__PURE__ */ new Map();
      if (Array.isArray(externalFragments)) {
        externalFragments.forEach((def) => {
          externalFragmentsMap.set(def.name.value, def);
        });
      } else {
        visit(parse$1(externalFragments, {}), {
          FragmentDefinition(def) {
            externalFragmentsMap.set(def.name.value, def);
          }
        });
      }
      const fragmentDependencies = queryEditor.documentAST ? getFragmentDependenciesForAST(queryEditor.documentAST, externalFragmentsMap) : [];
    ...
    try {
      ...
      const fetch2 = fetcher({
        ...
        shouldPersistHeaders,
        documentAST: (_a = queryEditor.documentAST) != null ? _a : void 0
      });
      ...
      if (isObservable(value2)) {
        ...
        (async () => {
          try {
            ...
            setIsFetching(false);
            setResponse(formatError(error2 instanceof Error ? error2 : new Error(`${error2}`)));
            setSubscription(null);
          }
        })();
        setSubscription({
          unsubscribe: () => {
            var _a2, _b;
            return (_b = (_a2 = value2[Symbol.asyncIterator]()).return) == null ? void 0 : _b.call(_a2);
          }
        });
      } else {
  ...
  }, [autoCompleteLeafs, externalFragments, fetcher, headerEditor, history, onEditOperationName, queryEditor, responseEditor, shouldPersistHeaders, stop, subscription, updateActiveTabValues, variableEditor]);
  const value = useMemo(() => ({
    isFetching,
    run,
    stop,
...
function SchemaContextProvider(props) {
  ...
  useEffect(() => {
    ...
    fetch2.then((result) => {
      ...
      if ((result == null ? void 0 : result.data) && "__schema" in result.data) {
        try {
          const newSchema = buildClientSchema(result.data);
          setSchema((current) => current === void 0 ? newSchema : current);
        } catch (error2) {
  ...
  }, [fetcher, introspectionQueryName, introspectionQuery, introspectionQuerySansSubscriptions, props.schema]);
  const validationErrors = useMemo(() => {
    if (!schema || props.dangerouslyAssumeSchemaIsValid) {
      return null;
    }
    const errors2 = validateSchema(schema);
    return errors2.length > 0 ? errors2 : null;
...
function useChangeHandler(editor, callback, storageKey, tabProperty, caller) {
  ...
  useEffect(() => {
    ...
    const handleChange = /* @__PURE__ */ __name$8((editorInstance) => {
  ...
  useEffect(() => {
    ...
    importCodeMirror([
      ...
      const newEditor = CodeMirror(container, {
        ...
        theme: editorTheme,
        keyMap: keyMap != null ? keyMap : "sublime",
...
function useQueryEditor({
  ...
  useEffect(() => {
    ...
    function getAndUpdateOperationFacts(editorInstance) {
      ...
      editorInstance.variableToType = (_e = operationFacts == null ? void 0 : operationFacts.variableToType) != null ? _e : null;
      if (variableEditor) {
        ...
        (_f = codeMirrorRef.current) == null ? void 0 : _f.signal(variableEditor, "change", variableEditor);
      }
    ...
    const handleChange = debounce(100, (editorInstance) => {
      ...
      if (onEditOperationName && (operationFacts == null ? void 0 : operationFacts.operationName) !== void 0 && editorInstance.operationName !== operationFacts.operationName) {
  ...
  }, [onEdit, onEditOperationName, queryEditor, schema, storage, variableEditor, updateActiveTabValues]);
  ...
  useKeyMap(queryEditor, ["Cmd-Enter", "Ctrl-Enter"], executionContext == null ? void 0 : executionContext.run);
...
function useSynchronizeExternalFragments(editor, externalFragments, codeMirrorRef) {
  useEffect(() => {
    ...
    const didChange = editor.options.lint.externalFragments !== externalFragments;
...
function EditorContextProvider(props) {
  ...
  const [storedEditorValues] = useState(() => {
    ...
    return {
      ...
      variables: (_f = (_e = props.variables) != null ? _e : storage == null ? void 0 : storage.get(STORAGE_KEY)) != null ? _f : null
    };
  });
  ...
  const initialValues = useRef({
    ...
    initialVariables: (_d = storedEditorValues.variables) != null ? _d : ""
  });
  const value = useMemo(() => __spreadValues(__spreadProps(__spreadValues({}, tabState), {
    addTab,
    changeTab,
    closeTab,
    updateActiveTabValues,
    ...
    setResponseEditor,
    setVariableEditor
  }), initialValues.current), [tabState, addTab, changeTab, closeTab, updateActiveTabValues, headerEditor, queryEditor, responseEditor, variableEditor]);
